 ...
 * @since 2017-01-07
 */
public class GaugeCustom extends View {

    private final int COLOR_GREEN = Color.parseColor("#00FF00");
    private final int COLOR_YELLOW = Color.parseColor("#FFFF00");
    private final int COLOR_ORANGE = Color.parseColor("#FF6D00");
    private final int COLOR_RED = Color.parseColor("#FF0000");
    private Paint needlePaint;
    private Path needlePath;
    private Paint needleScrewPaint;

    private boolean reverseNeedleColorOrder = false;

    private float needleStartAngle = -135f; // Góc bắt đầu mặc định của kim

    private boolean reverseNeedle = false;

    private float needleEndAngle = 135f; // Mặc định như gauge scale
    ...
    private float startAngle = -135f; // Góc bắt đầu của gauge
    private float endAngle = 135f;    // Góc kết thúc của gauge
    private float needleLengthRatio = 0.8f; // Tỉ lệ chiều dài kim trên bán kính gauge
private static final String TAG = GaugeCustom.class.getSimpleName();
    ...
    private float upperTextOffsetX = 0.5f; // Mặc định: chính giữa (50%)


    private float upperTextOffsetY = 0.2f; // Mặc định: 20% từ trên xuống
    ...
    public GaugeCustom(Context context) {
        super(context);
        initValues();
        initPaint();
    }

    public GaugeCustom(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public GaugeCustom(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void applyAttrs(Context context, AttributeSet attrs) {
        ...
        needleLengthRatio = a.getFloat(R.styleable.Gauge_needleLengthRatio, needleLengthRatio);
        needleStartAngle = a.getFloat(R.styleable.Gauge_needleStartAngle, needleStartAngle);
        reverseNeedle = a.getBoolean(R.styleable.Gauge_reverseNeedle, reverseNeedle);
        needleEndAngle = a.getFloat(R.styleable.Gauge_needleEndAngle, needleEndAngle);
        reverseNeedleColorOrder = a.getBoolean(R.styleable.Gauge_reverseNeedleColorOrder, false);
        ...
        upperTextOffsetX = a.getFloat(R.styleable.Gauge_upperTextOffsetX, upperTextOffsetX);
        upperTextOffsetY = a.getFloat(R.styleable.Gauge_upperTextOffsetY, upperTextOffsetY);
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        // Xác định phần trăm giá trị
        float percent = (needleValue - minValue) / (maxValue - minValue);

        if (!reverseNeedleColorOrder) {
            if (percent < 0.5f) {
                needlePaint.setColor(COLOR_GREEN);
            } else if (percent < 0.7f) {
                needlePaint.setColor(COLOR_YELLOW);
            } else if (percent < 0.9f) {
                needlePaint.setColor(COLOR_ORANGE);
            } else {
        ...
        drawLabels(canvas);
        drawTexts(canvas);
        canvas.rotate(needleStartAngle + valueToDegrees(needleValue), canvasCenterX, canvasCenterY);
...
//        canvas.drawOval(faceRect, rimShadowPaint);
    ...
    private void drawScale(Canvas canvas) {
        ...
        canvas.save();
        canvas.rotate(startAngle, canvasCenterX, canvasCenterY);
        for (int i = 0; i < totalNicks; ++i) {
            ...
            if (value >= minValue && value <= maxValue) {
                ...
                if (percent < 0.50f) {
                    scalePaint.setColor(COLOR_GREEN);
                } else if (percent < 0.7f) {
                    scalePaint.setColor(COLOR_YELLOW);
                } else if (percent < 0.9f) {
                    scalePaint.setColor(COLOR_ORANGE);
                } else {
                    scalePaint.setColor(COLOR_RED);
                }
                ...
                } else if (i % (majorNickInterval / 2) == 0) {
    ...
    private void drawLabels(Canvas canvas) {
        for (int i = 0; i < totalNicks; i += majorNickInterval) {
            float value = nickToValue(i);
            if (value >= minValue && value <= maxValue) {
                float scaleAngle = startAngle + (i * degreesPerNick);
    ...
    private void drawTexts(Canvas canvas) {
        float upperTextX = canvasWidth * upperTextOffsetX;
        float upperTextY = canvasHeight * upperTextOffsetY;
        drawTextCentered(upperText, upperTextX, upperTextY, upperTextPaint, canvas);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        scalePaint.setStrokeWidth(0.02f * canvasWidth);
    ...
    private void setNeedle() {
        ...
        float innerX = canvasCenterX + (needleLength * (1 - needleLengthRatio));

        needlePath.moveTo(innerX, canvasCenterY - (needleWidth / 2));
        needlePath.lineTo(canvasCenterX + needleLength, canvasCenterY);
        needlePath.lineTo(innerX, canvasCenterY + (needleWidth / 2));
        needlePath.lineTo(innerX, canvasCenterY - (needleWidth / 2));
        needlePath.close();

        needleScrewPaint.setShader(new RadialGradient(
    ...
    public void setNeedleStartAngle(float angle) {
        needleStartAngle = angle;
        invalidate();
    }
    public void setReverseNeedle(boolean reverse) {
        this.reverseNeedle = reverse;
        invalidate();
    }

    public void setNeedleEndAngle(float angle) {
        needleEndAngle = angle;
        invalidate();
    }

    public void setUpperTextOffsetY(float ratio) {
        upperTextOffsetY = Math.max(0f, Math.min(ratio, 1f)); // clamp từ 0 đến 1
        invalidate();
    }

    public void setUpperTextOffsetX(float ratio) {
        upperTextOffsetX = Math.max(0f, Math.min(ratio, 1f)); // clamp 0–1
        invalidate();
    }

    public void setReverseNeedleColorOrder(boolean reverse) {
        this.reverseNeedleColorOrder = reverse;
        invalidate();
    }
    ...
    private float valueToDegrees(float value) {
        ...
        float angleRange = needleEndAngle - needleStartAngle;

        float ratio = (clampedValue - minValue) / range;

        return reverseNeedle
                ? (1 - ratio) * angleRange
                : ratio * angleRange;
    }
    ...
    public void setNeedleLengthRatio(float ratio) {
        needleLengthRatio = Math.max(0f, Math.min(ratio, 1f));
        setNeedle();
        invalidate();
    }
